...
| `apps/web` | **exists, and is the largest tree in the repo — 1019 tests in 72 files.** That is not coverage: 66 of its 104 shipped modules are tested, and **38 have no test at all** — 11 of the 12 `components/shell/` modules, 11 of the 19 top-level `components/`, 6 of the 8 routes, both `queries/` read models, and every `stores/`, `command-palette/` and `keyboard/` helper. `components/ui/` (17 primitives), `components/data/` (10), `components/issue/` (6) and 4 of the 5 `lib/` modules are tested *and* reviewed. Run the count rather than trusting it: the previous figure in this row (677 in 50) was two units of work stale. `src/gallery/` is a second Vite entry at `/gallery.html` — 9 dev-only modules, outside `build.rollupOptions.input` and outside `ROUTE_PATTERNS`, so it never ships. `apps/web/README.md` §"State of this tree" holds the split — read it rather than this row before assigning UI work. Three tsconfigs, deliberately: `tsconfig.json` must include every file because it is the only name a language server discovers, `tsconfig.app.json` is the narrow one proving app code cannot import `node`. `e2e/` does not exist while `playwright.config.ts:42` points at it |
