 ...
 *
 * `h-9` (36px) on the same ladder as Button and SelectTrigger, so a filter row
 * of a search box, a select and a button has one baseline. It was `h-8`, and the
 * whole ladder moved up one rung when the type scale was measured off the
 * references — 16px text in a 32px box leaves 4px above and below it.
 *
...
const inputVariants = cva(
  [
    'w-full min-w-0 text-fg transition-colors duration-90 ease-out',
  ...
  ],
  {
    variants: {
      size: {
        ...
        default: 'h-9 px-2.5 text-base file:h-7 file:text-sm',
      },
      /**
       * Declared **after** `size` on purpose. cva concatenates variant groups in
       * key order, so `bare`'s `h-full p-0` has to come later in the string than
       * `size`'s `h-9 px-2.5` for tailwind-merge to resolve in its favour. With the
       * two groups the other way round a `bare` field would silently keep the
       * default height and padding, and nothing would say so.
       */
      variant: {
        default: [
          'rounded-control border border-border-control bg-surface shadow-xs',
          'focus-visible:border-ring',
          'aria-invalid:border-danger-accent',
        ],
        /**
         * No box at all — the sidebar's filter row in both references, where the
         * magnifier and the placeholder sit directly on the chrome with no border,
         * no fill and no radius. It fills its row (`h-full`) rather than declaring a
         * height, because the row it lives in is the thing that was measured.
         *
         * `text-md` because that row's type matches a project row's title beside
         * it, and the whole point of the variant is that the two read as one list.
         *
         * It carries no `aria-invalid` styling, and could not: `border-0` leaves
         * nothing for a colour to land on. That is the constraint that decides where
         * it may be used — a field that can be *wrong* needs a box to say so, so
         * this is for filters and never for form input.
         */
        bare: 'h-full border-0 bg-transparent p-0 text-md shadow-none',
      },
    },
